...
function updateList(contactsToDisplay = users) {
    ...
    contactsToDisplay.forEach((elem, index) => {
        ...
        contact.innerHTML = `
          ...
          <span class="infoBtn">
            <button onclick="addOrEdit(${index})">
               <img alt="icon" src="accests/edit contact icon.png" style="width: 15px; height: 15px;">
            </button>
            <button onclick="deleteUser(${index})">
              <img alt="icon" src="accests/delete contact icon.png" style="width: 15px; height: 15px;">
            </button> 
            <button onclick="infoPopup(${index})">
              <img alt="icon" src="accests/info icon.png" style="width: 15px; height: 15px;">
            </button> 
          </span>
        `;
        list.append(contact);
    ...
    message(); // Check if the array is empty and display a message if it is
}
updateList(); // Call with no arguments to load all contacts initially
...
function searchContacts() {
    ...
    for (let i = 0; i < users.length; i++) {
        if (users[i].name.toLowerCase().indexOf(searchQuery) !== -1) {
...
function infoPopup(index) {
    ...
    // Clearing up the previous content.
...
// Function to close the popup when clicking x or outside of the popup (the grey area).
